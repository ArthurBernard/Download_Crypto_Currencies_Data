...
- `dccd/histo_dl/exchange.py` — `save()` now supports `form='parquet'`; previously only `'xlsx'` and `'csv'` were handled (#XX)
- `scripts/backfill.py` — windowed backfill script for Binance, Kraken, and Bybit; loops in API-limit-sized windows and resumes from last saved timestamp (#XX)
- `config.yml` — ready-to-use daemon config for minutely OHLC + real-time orderbook/trades on Binance, Kraken, and Bybit (#XX)

### Fixed

- `dccd/histo_dl/exchange.py` — `save(form='parquet')` was silently ignored (logged a warning instead of writing the file) (#XX)
